...
function appraisemanager() {
    ...
    var listhref = "content/appraiselist.jsp";
    
    var modelhref = "content/appraiseform.jsp";

    //添加数据
    function addModel() {

        var options = {
            url: "appraise/add", //要访问的后台地址
            cache: false,
            type: 'post',
            dataType: "json",
            data: { "Method": "Add" },
    ...
    function loadCourses() {
        ...
        $.ajax({
            cache: false,
            type: "GET",
            url: "subcourse/getAllList",
            data: { "Method": "select"},
            dataType: "json",
            success: function (data) {
                $(data.records).each(function () {
                    for (var i = 0; i < $(this).length; i++) {
                        var option = $("<option>").text($(this)[i].subname).val($(this)[i].id);
  ...
  //根据ID加载对象
    ...
    function loadModels() {
        ...
        $.ajax({
            url: "appraise/getListbyPage", //要访问的后台地址
            type: "GET", //使用get方法访问后台
            cache: false,
            dataType: "json",
            data: params,
            success: function (response) {//msg为返回的数据，在这里做数据绑定
                if (response.records != null) {
                    ...
                    $.each(records, function (i, n) {
                        ...
                        row.find("#studentid").text(n.studentid);
                        row.find("#teacherid").text(n.teacherid);
                        row.find("#subcourseid").text(n.subcourseid);
                        row.find("#grade").text(n.grade);
                        row.find("#factscore").text(n.factscore);
                        row.find("#attitudescore").text(n.attitudescore);
                        row.find("#qualityscore").text(n.qualityscore);
                        row.find("#advise").text(n.advise);
                        var date = timeStamp2String(n.firetime.toString())
                        row.find("#firetime").text(date);
