...
public class TeacherController extends BaseController {
	...
	public Map<String, Object> getTeacherByConditions(HttpServletRequest request, Model model) {
		...
		List<Teacher> list = this.teacherService.selectBylimit(sqlwhere, (iPageIndex - 1) * iLimit,
				iPageIndex * iLimit);
		for (Teacher item : list) {
			StatOrder statorder = viewSuborderService.statByTeacherid(item.getId());
			item.setStatOrder(statorder);
	...
	public Map<String, Object> login(HttpServletRequest request, Model model) {
		...
		Teacher teacher = this.teacherService.selectByCode(accounts);
	...
	public Map<String, Object> show(HttpServletRequest request, Model model) {
		...
		// 通过教师ID查询子课程
		String sqlwhere = "teacherid='" + id + "'";
		List<SubCourse> list1 = this.subCourseService.selectBySql(sqlwhere);
		if (list1.size() > 0) {
			String url = list1.get(0).getRecordurl();
			if (!"".equals(url) && url != null) {
				url = url.replace("https", "http");
				map1.put("videoURL", url);
			} else {
				map1.put("videoURL", "");
			}
		} else {
		...
		map1.put("videoImage", "http://123.56.153.57:8080/education/images/students/student1.PNG");
		teacher.setVideodict(map1);
