...
public class AppraiseController extends BaseController {
	...
	public Map<String,Object> getRaiseListByTeacherid(HttpServletRequest request,Model model){
		...
		List<Appraise> list=this.appraiseService.selectBylimit(sqlwhere, (iPageIndex - 1) * iLimit, iPageIndex * iLimit);
		for(Appraise item:list)
		{
			...
			item.setStudentid(student.getRealname()==null?"":(student.getAccounts().substring(0,3) + "****" 
																+ student.getAccounts().substring(7
																, student.getAccounts().length())));
